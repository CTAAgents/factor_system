...
with tempfile.TemporaryDirectory() as tmpdir:
    ...
    for fid, name, sharpe, max_corr, partner in factor_configs:
        ...
        (elite_dir / f"{fid}.json").write_text(json.dumps(data, ensure_ascii=False, indent=2))
        code_hash = hashlib.sha256(f"factor_code_{fid}".encode()).hexdigest()[:8]
        meta_str = ", correlation_metadata 已写入" if partner else ""
    ...
    test_l2_prior = [
        ...
        {
            "factor_id_a": "factor_099",
            "factor_id_b": "factor_100",
            "pearson": 0.97,
            "spearman": 0.96,
        },  # 不存在于 test_factors
    ]
    ...
    print("  - factor_001 × factor_002 (存在于因子池中)")
    print("  - factor_005 × factor_006 (存在于因子池中)")
    print("  - factor_099 × factor_100 (不存在于因子池中)")
    print()
