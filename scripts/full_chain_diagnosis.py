...
def check_factor_code(factor_id: str, data: pd.DataFrame):
    ...
    if "close" in code and "open" in code:
        print(f"  ✅ 因子使用 close+open（正常）")
    if "high" in code or "low" in code:
        print(f"  ⚠️ 因子使用了 high/low（日内数据，需确认是否在当日可用）")
    ...
    local_vars = {"open": open_price, "high": high, "low": low,
                  "close": close, "volume": volume, "n": n, "np": np}
    try:
    ...
    if callable(factor_fn):
        ...
        print(f"  ❌ 未找到 factor_program 或 output 变量")
        return None
...
def check_signal_generation(factor_values: np.ndarray, strategy_returns: np.ndarray, positions: np.ndarray, data: pd.DataFrame):
    print(f"\n{'='*60}")
    print(f"🔍 [4/5] 信号生成与交易逻辑检查")
    print(f"{'='*60}")
    ...
    if ics:
        ...
        print(f"  ⚠️ 无法计算 IC 序列")


# ─── 5. 常见计算错误检查 ──────────────────────────────────

def check_common_errors(data: pd.DataFrame, factor_values: np.ndarray, strategy_returns: np.ndarray, positions: np.ndarray):
    print(f"\n{'='*60}")
    print(f"🔍 [5/5] 常见计算错误检查")
    print(f"{'='*60}")
