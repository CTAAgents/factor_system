        ...
        try:
            ...
            if "vwap" in code.lower():
                ...
                all_vwap_raw[sym] = {
                    ...
                    'volume': volume, 'high': high, 'low': low, 'signal': sig,
                }
...
def generate_summary_report(results: list[dict], today: str) -> str:
    ...
    lines.append("---")
    lines.append(f"详细报告见: factor_details/ 目录")
...
def generate_factor_detail_report(result: dict, today: str) -> str:
    ...
    if ss:
        lines.append(f"- 边界值占比 (|sig| >= 0.99): 平均 {ss.get('avg_boundary', 0):.1%}, 中位数 {ss.get('median_boundary', 0):.1%}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--factor-id", type=str, default=None,
                        help="单因子模式: 指定因子 ID (如 fct_d9a1d0fd)")
    parser.add_argument("--days", type=int, default=500,
                        help="数据窗口天数 (default: 500)")
