...
种子因子清单（来自 multi_factor_strategy.py FACTOR_WEIGHTS）:
    1. momentum              动量因子
    2. volatility_reversion  波动率回归
    3. volume_flow           资金流
    4. oi_change             持仓量变化
    5. basis                 基差
    6. inventory_pct         库存分位
    7. capacity              开工率
    8. macro_regime          宏观制度
    9. rate_proxy            利率代理
    10. pmi_proxy            PMI 代理
    11. position_rank        龙虎持仓
    12. warrant_change       仓单变化
...
def factor_program(data, params):
    ...
    chg[1:] = (close[1:] - close[:-1]) / np.maximum(close[:-1], 1e-10)
    score = np.where(
        (oi_ratio > 0.05) & (chg > 0), 0.6,   # OI增+价涨 → 多头
        np.where(
            (oi_ratio > 0.05) & (chg < 0), -0.6,  # OI增+价跌 → 空头
            np.where(
                (oi_ratio < -0.05) & (chg > 0), 0.3,  # OI减+价涨 → 多头
...
def factor_program(data, params):
    import numpy as np
    if 'inventory_pct' not in (data.columns if hasattr(data, 'columns') else data):
        return np.zeros(len(data['close']))
...
def factor_program(data, params):
    import numpy as np
    if 'capacity_pct' not in (data.columns if hasattr(data, 'columns') else data):
        return np.zeros(len(data['close']))
...
def factor_program(data, params):
    import numpy as np
    if 'macro_signal' not in (data.columns if hasattr(data, 'columns') else data):
        return np.zeros(len(data['close']))
    macro = data['macro_signal'].values if hasattr(data, 'macro_signal') else data['macro_signal']
    # bull → +0.5, bear → -0.5, neutral → 0
...
def factor_program(data, params):
    import numpy as np
    if 'rate_mom' not in (data.columns if hasattr(data, 'columns') else data):
        return np.zeros(len(data['close']))
...
def factor_program(data, params):
    import numpy as np
    if 'pmi' not in (data.columns if hasattr(data, 'columns') else data):
        return np.zeros(len(data['close']))
...
def factor_program(data, params):
    import numpy as np
    if 'top5_ratio' not in (data.columns if hasattr(data, 'columns') else data):
        return np.zeros(len(data['close']))
...
def factor_program(data, params):
    import numpy as np
    if 'warrant_change_pct' not in (data.columns if hasattr(data, 'columns') else data):
        return np.zeros(len(data['close']))
