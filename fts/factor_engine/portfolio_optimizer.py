...
class PortfolioOptimizer:
    ...
    def optimize(
        ...
        Args:
            ...
            prev_weights: 上一期权重 (n,)（启用换手约束时传入）
        ...
        if self._config.mode == "risk_parity":
            ...
            w = self._solve_risk_parity(cov, prev, returns)
        elif _HAS_SCIPY:
            w = self._optimize_scipy(cov, mu, prev, returns, exposure)
        else:
            logger.warning(
                "[L3-Opt] scipy 未安装，使用 numpy 降级优化（无约束精确解 + 投影）"
            )
            w = self._optimize_numpy(cov, mu, prev)
    ...
    def _optimize_scipy(
        ...
        def objective(w: np.ndarray) -> float:
            return -float(mu @ w) + lam * float(w @ cov @ w)
