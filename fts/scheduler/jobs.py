...
def _project_version() -> str:
    """读取 pyproject.toml 版本号（失败回退 unknown）。"""
    try:
        import tomllib
...
def _count_inject_dir(inject_dir: str | Path) -> int:
    """注入目录候选文件数（cand_*.json）。"""
    try:
        return len(list(Path(inject_dir).glob("cand_*.json")))
    except Exception:  # noqa: BLE001
        return 0


def _count_factor_pool(pool_path: str | Path) -> int:
    """因子池总数（factor_pool.json：total_count 优先，兼容 factors 列表）。"""
    try:
        data = json.loads(Path(pool_path).read_text(encoding="utf-8"))
        if isinstance(data, dict):
            if isinstance(data.get("total_count"), int):
                return data["total_count"]
            return len(data.get("factors") or [])
...
def _write_l1_report(
    *,
    ...
    completed_at: str,
    injected_count: int,
    factor_pool_total: int,
    inject_dir_count: int,
    note: str = "",
    report_dir: str | Path | None = None,
) -> Path:
    """生成并覆盖写入 L1 执行报告（memory/logs/meta_loop/YYYY-MM-DD.log）。

    由 l1_meta_loop_job 每次执行后自动调用（失败路径也写 failed 报告），
    使执行结果自包含落盘，不再依赖外层调用方手工生成。
    """
    base = Path(report_dir) if report_dir is not None else PROJECT_ROOT / "memory" / "logs" / "meta_loop"
    base.mkdir(parents=True, exist_ok=True)
    report = base / f"{datetime.now().strftime('%Y-%m-%d')}.log"
    lines = [
        "=" * 70,
        "L1 Meta-Loop 每日知识补给执行报告（全部期货 futures）",
        "=" * 70,
        f"执行市场: {market}",
        f"系统版本: {_project_version()}",
        "执行模式: l1_meta_loop_job 自动执行报告（覆盖当日旧日志）",
        "=" * 70,
        "",
        "【执行概要】",
        f"  状态: {status}",
        ...
        f"  启动时间: {started_at}",
        f"  完成时间: {completed_at}",
        f"  本次注入候选数: {injected_count}",
        f"  注入目录文件数: {inject_dir_count}",
        f"  因子池总数: {factor_pool_total}",
    ]
    if note:
        lines.append(f"  {note}")
    lines.append("=" * 70)
...
def l1_meta_loop_job(market: str | None = None) -> None:
    ...
    try:
        ...
        _write_l1_report(
            ...
            injected_count=len(result.injected_candidate_ids),
            factor_pool_total=_count_factor_pool(factor_pool_path),
            inject_dir_count=_count_inject_dir(inject_dir),
            note=f"命令: python -u -c \"from fts.scheduler.jobs import l1_meta_loop_job; "
            f"l1_meta_loop_job()\"（FTS_DEFAULT_MARKET={market}）",
        )
    ...
    try:
        ...
        for factor in all_elite_factors:
            try:
                ...
                # 用模拟 OHLCV 数据做行为漂移检测（补全 high/low/volume/settle/vwap 列，
                # 匹配因子代码的 K 线字段引用，避免 KeyError；settle/vwap 为期货扩展字段，
                ...
                mock_typical = (mock_high + mock_low + base) / 3.0
...
def factor_inspector_job() -> None:
    ...
    市场口径（v2.104.0+100）：跟随全局 FTS_DEFAULT_MARKET（energy/futures
    两套独立 catalog 各自巡检，v2.104.0+101 起替代固定 energy）。
    GAP-132（2026-08-20 关闭）：退化检测在评估历史不足时不再静默失效——
    detect_degradation 增加绝对水平兜底（最新 Sharpe < 1.0 / 最新 IC < 0 /
    最新评估未通过 → 判退化），恢复自动降级（commit=True）。
    """
...
def data_level_monitor_job() -> None:
    ...
    市场跟随全局 FTS_DEFAULT_MARKET：energy → 能化链（ENERGY_CHAIN_SYMBOLS）；
    futures → 全期货动态核心池（get_dynamic_core_subset，静态池兜底）。
    读取核心品种 DuckDB 缓存执行四维检查（复权一致性需第二复权源，暂以多源分歧
    覆盖；复权一致性检查由监控器接口保留，供对账流程调用）。
    ...
    try:
        ...
        from fts.data_futures import ENERGY_CHAIN_SYMBOLS, get_dynamic_core_subset
        market = _global_market()
        symbols = ENERGY_CHAIN_SYMBOLS if market == "energy" else get_dynamic_core_subset()
...
def factor_level_monitor_job() -> None:
    ...
    对因子库（市场跟随全局 FTS_DEFAULT_MARKET：futures → factor_catalog_futures.duckdb /
    energy → factor_catalog_energy.duckdb）执行三维检查：
    ...
    try:
        ...
        import duckdb
        market = _global_market()
        db_path = PROJECT_ROOT / "data" / f"factor_catalog_{market}.duckdb"
        if not db_path.exists():
