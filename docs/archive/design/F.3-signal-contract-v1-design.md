...
> 变更: 2026-08-20 增补 §8.1「定义分发通道」——双轨并存（自组装为主 + 信号矩阵作对账基准与降级），详细接口见 [F.4-factor-definition-dispatch-interface.md](F.4-factor-definition-dispatch-interface.md)
...
本契约（信号矩阵直读）与**定义分发通道**双轨并存，由 RD 在 2026-08-20 起按以下分工使用：

| 通道 | 交付物 | 用途 | 状态 |
|:--|:--|:--|:--|
| **定义分发**（F.4，主路径） | 因子 code+params（REST `/api/factors/definitions` / duckdb 直读兜底） | RD 本地 DataLoader 数据自组装计算，注册进 FactorRegistry 按 factor_id 引用 | 设计中（阶段 0 契约已固化） |
| **信号矩阵直读**（本契约，对账/降级） | 已计算信号矩阵（`l3_signal_matrix`） | ① 对账基准：证明"自组装信号 == FTS 信号"（diff < 1e-8）；② FTS 不可用时的降级通道 | 设计中（F.3） |

关键约束:
1. **同口径**：定义分发通道的因子 code 输出与信号矩阵同为"因子原始信号"（标准化/中性化在 RD 合成层做），详见 F.4 §3。
2. **状态同源**：两通道的 `factor_status` / `factor_scope` 均来自 FTS 因子库同一状态源，消费方状态映射见 F.4 §4。
3. **对账阻断**：对账偏差超限的因子不得进入生产因子池，退回本通道（F.4 §8）。
