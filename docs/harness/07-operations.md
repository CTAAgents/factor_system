...
| **v2.61.0** | **2026-08-09** | **L3/L4 机构级追赶 A 阶段（GAP-L301/L302/L305 首期，细则 plans/24-l3-l4-institutional-plan.md）：① GAP-L301 因子收益序列层——新增 `fts/factor_engine/factor_returns.py`（`FactorReturnsBuilder` 横截面多空组合收益序列构建，含 `align_to_factors`/`portfolio_returns`/`annualized_sharpe`/`max_abs_correlation` 组合层辅助），`build_combo` 新增 `factor_returns`/`annualize_factor` 参数：提供因子收益矩阵且可对齐（≥20 观测）时组合夏普/相关性由 w×R 实测（`metrics_source="measured"`），缺失或样本不足回退 diversity-adjusted 估算；`PortfolioCombo` 新增 `metrics_source` 字段；② GAP-L302 风险模型——新增 `fts/factor_engine/risk_model.py`（`RiskModelEstimator` 纯 numpy Ledoit-Wolf 收缩协方差：对角结构化目标 + 收缩强度估计，正定性保证 + 特征值/条件数/年化波动率输出，无 sklearn/scipy 硬依赖）；③ 专项计划落盘 + 08/09 登记（GAP-L 系列 11 项：P0×4 / P1×4 / P2×3）；④ 新增测试 21 用例（test_factor_returns 17 + test_portfolio_loop TestBuildCombo 实测化 4），相关回归全绿 |** |
