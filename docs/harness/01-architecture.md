...
EvolutionLoop（期货横截面因子演化，跨品种因子计算）
    │
    ▼
scripts/futures_signal_pipeline.py（横截面信号管道，方向校正 = 截面 IC 法，因子加权 = Ridge 回归 L2 正则化）
    │
    ▼
