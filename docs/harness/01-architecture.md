...
│  - L1→L2 闭环漏斗（plans/44 D1/D2，v2.104.0+86）：l1_l2_funnel.py —     │
│    state_kv `meta_loop/{market}/l1_l2_funnel` 累计 injected/            │
│    l2_consumed/l2_promoted（含时间戳）；回写三处：MetaLoop.run 注入 →   │
│    injected、L2 _merge_l1_candidates 消费 l1_injected_* → consumed、    │
│    L1 候选晋升 elite（parent_id cand_ 前缀）→ promoted；scripts/        │
│    l1_l2_funnel_report.py 每日输出转化率 + 积压 warning（>7 天未消费）  │
│                                                                         │
...
│  - load_elite_factors（加载 elite 因子：质量门槛 → 影子池冷却 → L2 评审│
│    approved 硬门槛（factor_reviews，rejected/未评审剔除，v2.104.0+88））│
│  - L3Verifier（L3 锁定协议）                                           │
...
- **L1 → L2**: 注入种子因子 + 演化方向指引（通过 seed_pool.inject()）
  - 闭环漏斗（plans/44 D1/D2，v2.104.0+86）：`l1_l2_funnel.py` 经 `state_kv meta_loop/{market}/l1_l2_funnel` 累计 injected（MetaLoop.run）→ l2_consumed（L2 消费 `l1_injected_*`）→ l2_promoted（L1 候选晋升 elite），`scripts/l1_l2_funnel_report.py` 输出转化率 + 积压 warning
...
| L2 批量子链评估 | 周日 09:00 | 每周 | **v2.105.0+16 新增**（`l2_subchain_quality_job`，subchain_eval.py）：全部 active 因子批量逐品种 IC → 子链画像 → 落库 `subchain_factor_quality` 质量矩阵（min_chain_ic=0.02，v2.105.0+16 由 0.10 校准），04:00 评审质检前刷新全量画像；无有效链因子不自动降级，metadata 标记 `pending_validation` 交进一步验证 |
| L2 评审质检统一任务 | 每日 04:00 | 每日 | **v3.0.0 合并**原「L2 周度评审 周日 10:00」+「评审质检阀门+三项监控 04:00」：**周日重量级分支**执行 `l2_review_job`（Step A reaudit 全量重审 → Step B 衰减评估 + AutoRetire 自动淘汰 → Step C `_review_gate_weekly` 阀门周度收口）；**其余日轻量五步**：① `_review_gate_weekly` pending 机审 + approved 复核 → ② `factor_inspector` 巡检降级（approved 豁免每日降级）→ ③ `logic_monitor` 逻辑监控 → ④ `data_level_monitor` 数据级监控 → ⑤ `factor_level_monitor` 因子级监控；全部在 05:00 前，新因子当日 approved 生效；energy 走 `l2_energy_qa_review_job` 手动调用（`FTS_ENERGY_QA_REVIEW_APPLY=1` 落库，v2.105.0+18 影子校验：无基线拒绝、同面板判定漂移拒绝） |
