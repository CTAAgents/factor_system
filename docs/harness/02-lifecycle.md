...
| **Phase 47** | 子链差异化与跨市场结构感知（v2.104.0+109，GAP-134/plans/47）：A 子链画像+显著性护栏（`subchain_profile.py`，三门槛 AND t 检验）+ A2 落库（`factor_catalog.metadata.subchain_ic_profile/scope/specific`）；B L3 子链差异化权重（`subchain_weight.py` 调制矩阵 + Step 2b + `factor_weights.json` 输出 `subchain_weights/symbol_chain` + 信号管线按品种调制 + CLI `--enable-subchain-weight`，灰度默认关）；C 期货/股票评估口径分离（`STOCK/FUTURES_EVAL_CONFIG` + `get_eval_config`）；D 子链暴露监控（Step 2b 告警 + 质量报告 `subchain_exposure` 段）；新增 50 测试用例 | ✅ 完成 | 3 新测试文件 50 用例全绿 + 受影响回归 390 passed；评估输出 `subchain_ic_report` 段；elite 因子携带子链画像 |
| **Phase 48** | Regime 分层方向 Gate 与品种暴露缩放（v2.104.0+111，GAP-136/plans/48）：A 子链方向 Gate（`regime_gate.py` `build_subchain_gates`/`apply_subchain_gate`，long/short/avoid/neutral + 盲测池默认回避；信号管线 Step 3h1 仅 energy 生效，失败降级）；B 品种暴露缩放（`map_confidence_to_exposure` 分段 0.4/0.7 + `apply_exposure_scale` = 置信度映射×品种-链对齐度，防双重惩罚）；C 收益来源族激活（`regime_adaptive_weight_adjustment(subchain_regimes)` 按 `subchain_scope` 路由子链 regime 倍率，数据不足回退全局 + `build_subchain_return_source` 画像入 `_regime_meta`）；D 灰度（`--enable-regime-gating` 默认关）+ 与 plans/47 正交串联验证 + 质量报告 `subchain_gate_distribution` 段；新增 36 测试用例 | ✅ 完成 | 3 测试文件 36 用例全绿 + 受影响回归 694 passed + ruff 通过；子链 Gate 分布/收益来源族激活画像入报告 |
| **Phase 49** | 因子×子链质量矩阵：评审质检与生命周期张量化（v2.104.0+112，GAP-137/plans/49）：A 存储底座（`subchain_factor_quality` 表 + `SubchainQualityRepository` + `build_subchain_quality_rows`，评估单元=(factor_id,market,chain)，晋升写首行/评审重算，E.4 短连接）；B 评审张量化（`judge_q10_subchain` Q10/F6 两级判定——外层跨产业链方向一致+内层子链特异 t 检验护栏+反向链 avoid；`AutoReviewPolicy` 单链特异放行；准入三级权重 `SUBCHAIN_SPECIFIC_MAX_WEIGHT=0.10`）；C 生命周期张量化（`compute_subchain_degradation` 单元粒度退化——全部有效链衰减→degrade/部分链→scope_shrink/单链特异因子唯一链衰减→degrade + `_shrink_scope` 剔除失效链更新 metadata.subchain_scope→Step 2b 调制矩阵自动重算闭环）；D 闭环+监控（质量报告 `subchain_quality_matrix` 段，与 47 `subchain_exposure`/48 `subchain_gate_distribution` 三网合一；子链定义参数化可扩展至其它产业链）；灰度 `l3.subchain_quality.enabled=false` 默认关回退全链原逻辑；新增 46 测试用例 | ✅ 完成 | 3 新测试文件 46 用例全绿 + 受影响回归 448 passed + ruff 通过；GAP-137 登记并关闭 |
| **Phase 50** | L3 权重层 Gate 闭环（v2.104.0+113，GAP-138/plans/50）：A `regime_gate.py` 新增 `gate_scale_map(gates, config)` 纯函数——Gate 决策 → 链级缩放系数（avoid+hard→0.0 / avoid+soft→soft_avoid_ratio / long·short·neutral→1.0 不干预：方向过滤属信号层 Step 3h1 职责）；B `portfolio_loop.py` Step 2.5 接线 `_merge_gate_scale_into_modulation`——Gate 开启 + energy + 子链 regime 检测成功时将 m'[factor][子链] = m × gate_scale（avoid 链权重源头归零/降权），同步 signals 的 subchain_weights 标注与 factor_weights.json 输出；依赖 Step 2b 调制矩阵存在（enable_subchain_weight）否则保持观测语义零行为变更；质量报告新增 `subchain_gate_scale` 段（与 subchain_exposure/gate_distribution/quality_matrix 四网合一）；新增 11 测试用例 | ✅ 完成 | 2 测试文件 +11 用例全绿（TestGateScaleMap 5 + TestMergeGateScaleIntoModulation 6）+ 受影响回归 358 passed + ruff 通过；GAP-138 登记并关闭 |
| **Phase 27** | CTA 手册 WorkFlow 端到端工作流 UI（v2.104.0+25）：`fts/workflow/` 子包（11 阶段 + 质检闭环 12 节点定义 + WorkflowExecutor 单阶段/端到端真实执行 + WorkflowStore SQLite WAL 持久化）+ `fts ui` 托管 `web/workflow_ui` React SPA（阶段节点流/批次历史/阶段详情日志+JSON产物/质检看板）+ 27 个缺 params 种子因子修复 + 28 用例 | ✅ 完成 | WorkFlow UI（/workflow）+ data/workflow.db 批次回放 + tests/workflow 28 用例全绿 |
...
| **Phase 48** | 双系统切分（v3.0.0，plans/57）——FTS 角色重定位为**因子生产系统**，策略合成职责迁移外部 Regime-Driven：① 信号契约 v1 固化（design/F.3，l3_signal_meta 追加 schema_version/factor_status/factor_scope 三列 + 历史回填 + 决策/训练双模式隔离）；② RD 承接（strategy_synthesis/combo_verifier/money_management/crowding_gate 权威替换/backtest_engine 消费信号矩阵/L2 子链化 5 子链）；③ FTS L3 组合侧**退役登记**（retired_l3.py 35 项 + import 期 DeprecationWarning + warn_if_retired 告警，存量调用点兼容不删码；物理删除为后续独立里程碑）；④ 全期货覆盖规划（config/futures_universe.yaml coverage_priority P0-P3，84 品种/17 产业链）；⑤ 存量因子集中重审管道（review_legacy_factors.py 分族 FDR-BH）；⑥ 验收（2026-08-20）：阶段 0 A/B（状态一致率 92.04% / 方向一致率 97.55%）+ 阶段 1 双轨对账（信号余弦 1.0000 / 组合方向 100% / 绩效差 0.000000）全门槛通过 + 全量回归 8129 passed（残留 21 项预存，登记 GAP-158） | ✅ 完成（阶段 0-13） | plans/57-dual-system-factor-strategy-split-plan.md + design/F.3-signal-contract-v1-design.md + fts/factor_engine/retired_l3.py + scripts/scan_l3_retirement.py |
