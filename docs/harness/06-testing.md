...
│   ├── test_subchain_weight.py      # L3 子链差异化权重调制测试（plans/47 §B5/§5.2，21 用例：build_subchain_weights scope 语义——单链·多链·all·unknown·缺省 scope_default·zero/soft 双模式·soft_min_ratio 地板；apply_subchain_modulation——3D 矩阵逐品种左乘·未知链兜底·ndim 校验·维度不匹配回退；L3 集成——inject_to_fdt 序列化 subchain_weights/symbol_chain·兼容不输出·PortfolioSignal 契约·信号管线按品种调制·_load_l3_subchain_meta；compute_chain_exposure 暴露占比）
