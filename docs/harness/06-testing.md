...
| `tests/test_factor_db_isolation.py` | 4 | 测试因子库隔离验证（GAP-129，v2.104.0+79 新建）：get_db_path 重定向至 tmp 且 futures/energy 分库路由保留 / 4 仓储 `_db_path` 落 tmp / 晋升写入三表（catalog+quality+audit）后真实库三表 COUNT 不变（零污染）/ uses_real_factor_db 豁免标记仍路由真实库路径 |
