...
| `tests/test_futures_signal_pipeline.py` | 41 | 信号管道 Ridge 回归加权 + 方向校正 + 组合合成 + delta 增量分类（v2.104.0+7）+ 加载源（v2.104.0+7）+ 单因子权重上限（v2.104.0+11：cap 截断/None 不截断/低于等权回退等权/单因子忽略） |
