...
│   ├── test_owl_factor_selector.py  # OWL 因子分组筛选器测试（plans/41 方案 A，21 用例：make_owl_weights 权重非递增/OWL 求解——强相关分组还原·稀疏·切割·归一尺度/组内正交检验显著·非显著·退化/边界——空·全 NaN·单因子·超短样本·退化/依赖缺失 cvxpy·scipy 降级 applied=False）
│   ├── test_owl_l3_integration.py   # OWL L3 旁路集成测试（plans/41 Step 1.8c，14 用例：owl_config 契约映射/enabled=false 零调用零行为变更/enabled+report_only 产出报告·不修改 factors/失败回退不阻断/_run_owl_sidecar 正常·降级·输入不足·落盘）
│   ├── test_subchain_profile.py     # 子链适用性画像与显著性护栏测试（plans/47 §A3/§5.1，24 用例：三门槛 AND 判定链——同向一致·方向混杂·弱均值·品种不足(n=1/2)·NaN 剔除·std=0 兜底(强/弱)·阈值参数化·ddof=1·p 值留痕·部分品种缺失·全负 IC 对称；scope 派生——单链 specific·多链·all·unknown·未知链键；A2 落库——metadata 三字段·inf JSON 安全·空 symbol_ic·懒加载；随机噪声误报率 <5%）
│   ├── test_subchain_weight.py      # L3 子链差异化权重调制测试（plans/47 §B5/§5.2，24 用例：build_subchain_weights scope 语义——单链·多链·all·unknown·缺省 scope_default·zero/soft 双模式·soft_min_ratio 地板；apply_subchain_modulation——3D 矩阵逐品种左乘·未知链兜底·ndim 校验·维度不匹配回退；L3 集成——inject_to_fdt 序列化 subchain_weights/symbol_chain·**键归一 factor_id→name**·无 factor_id 兼容·兼容不输出·PortfolioSignal 契约·信号管线按品种调制·_load_l3_subchain_meta·**端到端键链路（build→inject→load→composite 调制生效）**；compute_chain_exposure 暴露占比）
│   ├── test_eval_config_by_market.py # 评估口径按市场分离测试（plans/47 §C1，5 用例：FUTURES/STOCK_EVAL_CONFIG 阈值分离·别名保持现状·get_eval_config 路由 futures/energy/stock/未知·energy FactorVerifier 可实例化）
...
│   ├── test_subchain_eval.py        # 批量子链质量评估工作流测试（v2.105.0+16，5 用例：run 全流程落库+metadata 更新/无 IC 跳过/unknown scope 标记 pending_validation 不降级/空因子列表/单因子异常不阻断批量）
...
│   ├── test_meta_loop.py            # L1 元循环测试（plans/41 +9 用例：子链分批/实时链知识/LLM 分批/预算配置；GAP-131 +6 用例：拒绝候选落盘——rejected_dir 派生/显式覆盖/硬失败落盘含 code+原因/软失败重写未过落盘/JSON 可解析）
│   ├── test_meta_loop_phase3.py     # plans/44 Phase 3 bootstrap 优化测试（7 用例：C4 语义去重拦截与开关/B1 负面样本注入 prompt/C3 bootstrap narrative 补全）
│   ├── test_l1_l2_funnel.py         # plans/44 Phase 3 L1→L2 闭环测试（9 用例：漏斗累计/转化率/积压 warning/MetaLoop.run 注入回写/SeedManager 消费与晋升回写）
│   ├── test_bulk_collector.py       # plans/44 全球多源批量采集测试（14 用例：增量去重/arXiv/OpenAlex 多语种分路/东财/日韩法研报/计数契约/nonen 开关）
│   ├── test_knowledge_filter.py     # plans/44 粗筛测试（embedding/关键词降级/语义去重阈值）
...
**测试因子库隔离（GAP-129，v2.104.0+79）**：根 `tests/conftest.py` 注册 `uses_real_factor_db` 标记并提供 autouse fixture `_isolated_factor_db`——在单挂载点 `fts.factor_engine.factor_db.schema.get_db_path` 将 futures/energy 全市场重定向至每测试独立 tmp DuckDB（4 仓储类构造时局部导入该符号，一改全生效；futures/energy 分库文件名保留仅位置迁移），常规测试不再写真实 `data/factor_catalog_*.duckdb`（零污染，运行 test_evolution_loop 242 用例后真实库三表 COUNT 不变）。真实路由断言 / 真实存量因子数据依赖测试须显式标记 `@pytest.mark.uses_real_factor_db` 豁免（已豁免 5 处：test_energy_chain/test_factor_db/test_cli_extra/test_evolution_loop/test_bincount_boundary）。

**CI 零污染护栏（GAP-129，v2.104.0+80，plans/43 §4.3）**：`scripts/verify_factor_db_untouched.py` 提供 `--mode snapshot|check`（真实库三表 COUNT + 行级 md5 内容指纹对比；库不存在记录 absent，基线 absent → 当前 present 判为差异）。已接入 `.github/workflows/ci.yml` `test` job：`pytest` 前 snapshot、后 check——任何测试（绕过隔离显式传真实路径/直接用常量）写真实 `data/factor_catalog_*.duckdb` → 差异 → CI 失败。护栏路径直接读 `schema.DATABASE_PATH_*` 常量（不走 get_db_path，避免被隔离 fixture 架空）。
...
| `tests/monitor/test_data_level_monitor.py` | 28 | 数据级质量监控（GAP-F06 + GAP-085 + GAP-148）：缺失率/异常值/复权一致性/多源分歧 + 阈值边界/冷却/回调 + scheduler 接入 + hold 缺失检测/无 hold 列跳过 + 双符号完整度去重/adj_factor 派生豁免/sync 增强源注册（v2.101.0 + v2.105.0+17） |
...
| `tests/factor_engine/test_causal_validator.py` | ~15 | 因果结构审查（自然实验/预测误差；v2.105.0+4 新增 params 直取型回归用例——`params['window']` 直索引因子 validate 不抛 KeyError，锚定执行参数与评估链 `factor.get("params", {})` 口径一致） |
...
| `tests/monitor/test_logic_monitor.py` | 20 | 逻辑监控仪表盘（漂移检测/极端预测/换月日；v2.104.0+72 +4 用例：离散三态突破因子不误报/退化离散信号告警/连续信号保持 zscore/离散阈值可配置；v2.105.0+10 +1：DSL 因子引用 settle 列含 settle 模拟数据可正常执行——回归 fct_4d57a00f KeyError） |
| `tests/factor_engine/test_portfolio_loop.py` | 268 | L3 组合循环（含粘性约束 5 + 漂移监控 7 + 影子池 6 + 过拟合保护 6 + 股票 L3 组合层 6（GAP-I301）+ plans/28 28-T4/T6/T10 用例扩增：exposure_scale 计算与消费/RegimeSmoother 不对称/指标上报接线 + v2.103.0+5 _build_factor_code_map 4 用例：内存 code 优先/DuckDB 补拉/JSON 快照兜底/全缺空映射 + **方案③① 双指标+实测自动构建（v2.104.0+2，8 用例：signal_sharpe 双指标 4——缩放分离/无缩放相等/measured 相等/空组合 None + _auto_build_factor_returns 自动构建 4——矩阵构建/数据不足回退/空面板/无 code 回退）+ GAP-122（v2.104.0+42，4 用例：_verifier_view 替换 signal_sharpe/缺失原样返回/风控缩放组合经视图通过/低信号质量仍失败）** + **plans/36（v2.104.0+50，10 用例：TestFactorCompositeScore 5——综合评分归一化/缺失维度剔除重归一/权重排序/降级回退/配置透传 + TestQualityWeightMode 3——quality_weight 定权下限/权重归一/等权等价 + TestRollingOos 2——滚动 OOS 组合收益/decay_ratio 与数据不足降级）** + **子链维度去冗余（v2.104.0+60，6 用例：TestChainDedup——同链超限按综合评分截断/各链≤上限全保留/symbol_ic 缺失归 unknown 保留/max_per_chain=1 每链单因子/空输入/elite JSON symbol_ic 兜底读取）** + **risk_parity 配置化（v2.104.0+62，2 用例：test_run_optimizer_auto_matrix——optimizer 无显式矩阵自动构建仅用于权重合成+指标保持估算 / test_synthesize_optimizer_risk_parity——risk_parity 权重非负且 ≤1 非全仓特征）** + **L2 评审硬过滤（v2.104.0+88，+6 用例：TestLoadEliteDuckdb +2——评审驳回剔除/未评审剔除；TestFilterReviewApproved +4——approved 保留/rejected 剔除/unreviewed 剔除/空输入）**） |
| `tests/factor_engine/test_portfolio_loop_adaptive.py` | 31 | L3 adaptive 权重（v2.56.0 8 用例 + plans/28 28-T3/T6 扩增：probability_mix 概率混合/平滑参数契约 + **GAP-095 blend_power 幂次调节 6 用例 + GAP-094 calibration_path 接线 4 用例（v2.103.0+34）**）+ **plans/48 §C 子链收益来源族激活路由（v2.104.0+111，+9 用例：单链 scope 字符串/单元素列表路由子链 regime 倍率/all·unknown·部分链·未知链回退全局/缺省 subchain_regimes=None 与现状逐位一致/子链 regime_probs 概率混合/高波衰减按因子路由子链判定/build_subchain_return_source 画像结构与空输入）** |
| `tests/factor_engine/test_regime_gate.py` | 26 | 子链方向 Gate + 暴露缩放（plans/48 §A/§B，v2.104.0+111，新建）：build_subchain_gates 判定（bull/bear 高置信 long/short、低置信 avoid、oscillate·缺检测 neutral、min_confidence 参数化）+ apply_subchain_gate（hard 剔除/soft 降权/long·short 方向过滤/blind_default 双态）+ map_confidence_to_exposure 分段映射（<0.4→0/=0.4·0.7 边界/线性中点/≥0.7→1.0/参数化门槛）+ apply_exposure_scale（置信度映射×品种-链对齐度/score=0 与 avoid-soft 防双重惩罚/对齐度缺失保守 0.5）+ build_symbol_chain_map 反查 |
| `tests/factor_engine/test_subchain_weight.py` | 22 | L3 子链差异化权重调制（plans/47 §B5，v2.104.0+109）：build_subchain_weights scope 语义（all/unknown/单链/部分链/缺省 + zero/soft 双模式 + soft_min_ratio 保底）+ apply_subchain_modulation（3D 信号矩阵逐品种左乘/未知链兜底/维度校验）+ compute_chain_exposure + _load_l3_subchain_meta 读写；**plans/48 §D2 串联正交（v2.104.0+111，+1：幅度层调制仅动因子权重、方向层 Gate 仅动品种得分，正交不冲突）** |
...
| `tests/scheduler/test_tasks.py` | 39 | 调度任务 |
...
| `tests/test_cli.py` | ~65 | CLI 入口（v2.104.0+101 全局市场跟随 +1：portfolio run --universe 未指定时默认跟随 FTS_DEFAULT_MARKET） |
...
| `tests/test_futures_signal_pipeline.py` | 50 | 信号管道 L3 组合权重严格模式（v2.105.0：正常/缺失退出/损坏退出/空权重退出）+ L3 组合因子过滤（缺失跳过/全缺失退出/market-db_path 链库路由透传 v2.104.0+41）+ 因子类别分类 + Regime 档位缩放 + 品种级 IC 自适应 + 组合合成（方向校正恒空 dict）+ delta 增量分类（v2.104.0+7）+ 加载源（v2.104.0+7）+ TestHoldoutValidationAlignment（v2.104.0+81 GAP-130：短历史新品种盲测 IC/品种-因子 IC 对齐修复 ≈1.0 + 全历史品种零漂移 + 盲测池 min_rows 历史门槛跳过）；Ridge 权重与方向校正测试已随 v2.105.0 移除 |
...
| `tests/factor_engine/test_regime_profile.py` | 19 | Regime 画像与制度序列构建（plans/53 §A/§C/§D 新增）：画像三门槛（单/多/全/unknown scope、胜率口径、regime_dependent、min_abs_ic 门槛、长度不匹配安全、metadata 落库）+ RegimeSeriesBuilder（合成 OHLCV 滚动检测/数据不足）+ 晋升门槛 regime_gate_passed（无画像放行/单制度拦截/双制度放行/all·unknown 放行/参数化）+ 评估链 regime_ic_report 报告段（开关关零变更/energy 开启产出） |
| `tests/factor_engine/test_regime_conditional_weight.py` | 10 | Regime 条件化权重（plans/53 §B 新增）：zero 归零/soft 缩放/scope all·unknown 保留/无画像保留/弱负向保留/正向保留/effective 制度保留/当前制度无记录保留/非数值 IC 保留 |
...
| `tests/factor_engine/test_regime_beta_layer.py` | 13 | L0 宏观 Beta 层（plans/55 §A 新增，v2.105.0+22）：三态判定（牛面板 RISK_ON/熊面板 RISK_OFF/震荡面板 RANGE_BOUND）/空数据·单品种 unknown + scale=1.0 零行为变更/置信度门槛降级 RANGE_BOUND/股债比 z 符号（上行/下行）/beta_scale 映射（off/on/其余 1.0）/多空不对称偏置（RISK_OFF 多头 ×0.6·空头 ×1.2、RISK_ON 相反、neutral 不干预）/股债比缺失 2 信号降级不崩溃 |
| `tests/factor_engine/test_regime_crowding.py` | 16 | 拥挤度体系化（plans/56 §A 新增，v2.105.0+25）：6 信号分别触发（corr 趋近/放量滞涨/动量衰竭/波动突升/OI 天量/换手透支）/合成 score 加权/方向分解（多头·空头拥挤/中性，逼空识别修复 G3）/空面板 fallback/缺失 hold 降级不崩溃/联合门控矩阵（高置信高拥挤 0.5·低置信高拥挤 0.0·其余 1.0）/多空方向抑制（long 减多不抢反弹·short 减空不追空·neutral 不干预） |
| `tests/factor_engine/test_regime_risk_params.py` | 19 | G14 杠杆/止损止盈参数随 Regime 变化（原 11 + plans/55 §D 新增 8）：resolve_risk_params 表解析与回退 + 切换平滑 + RiskManager 注入 + MhfRiskConfig.from_regime + **Beta 档位叠加（RISK_OFF 在 bear/high_vol 上乘性收紧 0.7、RISK_ON 无约束、未知态保持、BETA_RISK_PARAMS 表结构）+ RiskManager beta_state 注入（RISK_OFF 收紧 / RISK_ON 无额外 / None 与现状一致）** |
...
| `tests/data_sources/test_tqsdk_enhance_source.py` | 15 | 天勤 TQSDK 字段增强源（GAP-083 阶段 C，v2.101.0）：close_oi→hold+差分→oi_change/open_oi 回退/无持仓字段 None/零值→NaN/无账号 None/无映射 None/天勤异常 None + 符号映射 + is_available 三态；+1（v2.105.0+28 GAP-154 IM0 映射补全：`_SYMBOL_MAP` 增 `IM0→KQ.m@CFFEX.IM`，增强层 IM0 hold 取数恢复） |
...
| `tests/data_sources/test_tqsdk_source.py` | ~19 | 天勤 TQSDK 数据源（周期/探活/映射/fetch_ohlcv 全路径/认证）（v2.47.0）；+1（v2.105.0+2 GAP-140⑤ 兼容层：`_import_tqsdk_safe` 后 TqApi 运行期 logger 接受裸 kwargs（api.py:3612 product= / auth.py:114 user_name=），loggerClass 还原为 `_TqCompatLogger` 标准 Logger 子类） |
...
| **合计** | **5377+** | plans/28 追加：test_regime.py →86（T1~T6 扩增）、test_regime_hmm.py →47（T2 扩增）、新增 test_regime_calibration.py 3 + test_regime_model_selection.py 2 + test_regime_validation.py 3、test_portfolio_loop.py →217、test_portfolio_loop_adaptive.py →12、test_config_settings.py →65、test_prometheus_metrics.py →42（T10 +4）；plans/29 P1 追加：test_migrate_elite_json_to_catalog.py 17；plans/29 P2 追加：test_state_db.py 11 + test_migrate_state_to_duckdb.py 8；plans/29 P3-A 追加：test_factor_optimizer.py →51（TestFactorSignalCacheParquet +5）；plans/29 P3-B 追加：test_archive_history_cold.py 7；GAP-091 阶段 2 追加：test_data_futures_fundamental.py 36→40（+4：东财路由/INE/EC 空/映射覆盖）；E.3 S2 追加：test_state_db.py 11→14（+3 SQLite 特性） + test_migrate_state_to_sqlite.py 6；E.4 S1 追加：test_duckdb_lock.py 4；GAP-117 追加：test_gap_threshold_calibration.py 7；**plans/36（v2.104.0+43）追加：test_portfolio_loop.py 227→237（+10：综合评分 5 + quality_weight 3 + 滚动 OOS 2）+ test_factor_clustering.py 43→48（+5：TestSelectRepresentativeScoreMap——score_map 优先于 sharpe/回退兼容/top-2 低相关保留/相关≥0.5 拦截/run 透传）**；**plans/37（v2.104.0+54~+57）追加：test_rolling_native.py 28 + test_seed_ops_native.py 56 + test_ops_native_batch3.py 48 + test_panel_vector.py 30→31（Phase 3 默认开启回归）**；**v2.104.0+77（GAP-121 扩展）追加：test_portfolio_loop.py 261→262（+1：energy 市场 Step 0.5 面板收缩至能源化工 20 品种（ENERGY_CHAIN_SYMBOLS ∪ ENERGY_CHAIN_HOLDOUT），futures 市场保持默认全池）**；**plans/38 + plans/40（v2.104.0+59~+63）追加：test_numba_kernels.py 32→86（+54：ts_rank 内核扩展 + plans/40 C 层 ts_zscore/ts_cvar 1D 重新接入）+ test_l3_signal_service.py 新增 16（plans/40 B/D 层：3D 构建/DuckDB corr/增量/持久化）**；**plans/51（v2.104.0+，衔接缺口修复）追加：test_l3_signal_service.py 16→21（+5：params 双哈希判定/向后兼容/load 契约/形状防护×2）+ test_config_settings.py +3（l3_signal_store 默认/覆盖/l3_signal_cache_entries）+ test_portfolio_loop.py +4（signal_store 自动激活/关闭/显式优先/end_date 推导）+ test_signal_cache.py evictions 可观测**；**plans/52（v2.104.0+，GAP-139 增量窗口追加）追加：test_l3_signal_service.py 21→26（+5：增量追加 vs 全量逐位一致/rolling 算子窗口回退/前缀不一致降级/验证失败兜底/旧库无 digest 兼容）+ test_config_settings.py +1（l3_signal_store_append_window 默认/覆盖）**；**plans/57（v3.0.0 双系统切分）追加：test_quantdata_provider.py 17（QuantData Provider 只读直读/品种映射/复权策略/期限结构权威构建）+ test_import_pipeline.py 4（外部因子导入管道）+ test_structure_constraints.py 15（演化挖掘期货结构约束）+ test_retired_l3.py 8（L3 退役登记/warn_if_retired 告警）+ test_verify_factor_mapping.py 5（RD 11 因子 ↔ FTS DSL 映射 Spearman 门槛）+ test_reconcile_dual_track.py 9（阶段 1 双轨对账）+ test_review_legacy_factors.py 11（P0 存量因子集中重审分族 FDR-BH）+ test_regime_premise_check.py 5（high_vol 前提交叉验证）+ test_universe_config.py 9→13（+4：coverage_priority 四级 P0-P3/级别互斥/并集=universe/T0 TL0 增补）+ test_l3_signal_service.py 26→34（+8：schema_version/factor_status/factor_scope 三列幂等迁移 + load_signal_meta + backfill_signal_matrix 版本锁定双哈希/工作区隔离 + verify_backfill_consistency 拼接校验）+ test_operator_expansion_d12.py（+ts_adx_wilder/ts_atr_ratio 算子精确复刻 RD 口径）**；**2026-08-20 全量回归实测：8129 passed / 21 failed（残留 21 项全部预存——GAP-157 严格注册表×测试隔离已修复清零、universe 测试已同步 84/14/9，剩余 GAP-158：工作区既有 portfolio_loop 本地改动 + pandas-3 只读 + regime_features，均非本次计划引入）**；**v3.0.0+10（2026-08-20）运维脚本测试补充 +89：test_rhi_global_setup.py 23 + test_cleanup_push_scope.py 40 + test_seeds_yaml.py 26，全量 8303 tests collected** |
|
