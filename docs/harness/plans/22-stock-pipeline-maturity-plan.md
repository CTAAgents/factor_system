...
#### GAP-S03 无 A 股行业轮动 / 风格轮动 Regime 检测（P1）
...
#### GAP-S09 种子表达式与 expr_dsl 编译链双轨（P1）
...
| v2.61.0 | 阶段 B | GAP-S03 | A 股行业轮动 + 风格轮动 Regime（StockRegimeSelector + HMM 集成） |
| v2.61.0 | 阶段 B | GAP-S04 | 股票信号管道升级（方向校正 + Ridge 权重学习 + 成本约束） |
| v2.62.0 | 阶段 C | GAP-S05 | A 股特有种子扩充（北向/两融/分析师/股东户数） |
| v2.62.0 | 阶段 C | GAP-S06/S07/S08 | 分层 IC + 仅做多口径 + 长窗口参数化 |
| v2.63.0 | 阶段 D | GAP-S09/S10 优先 | 表达层统一（种子走 DSL 编译链）+ 算子注册表合并（单一事实源） |
| v2.63.0 | 阶段 D | GAP-S11 | 股票演化 operator-first（算子演化为主干，LLM/GP 兜底） |
| v2.64.0 | 阶段 E | GAP-S12/S13 | A 股特有算子扩充 + 表达类型分布监控 |
...
| 字段 | 值 |
|:-----|:----|
| 代码→文档映射 | `fts/factor_engine/evaluation_chain.py`；`fts/factor_engine/evolution_loop.py`；`fts/factor_engine/regime.py`；`fts/factor_engine/regime_hmm.py`；`fts/factor_engine/portfolio_loop.py`；`fts/factor_engine/extractors/stock_pipeline.py`；`fts/factor_engine/seed_loader.py`；`fts/factor_engine/feature_ops.py`；`fts/factor_engine/expr_dsl/`；`fts/factor_engine/gp_evolver.py`；`fts/factor_engine/operator_evolution.py`；`fts/factor_engine/barra/`；`fts/config/settings.py`；`fts/data_fundamental.py`；`scripts/daily_signal_pipeline.py`；`scripts/futures_signal_pipeline.py`；`seeds/stock/`；`data/industry_map.json` |
| 可验证断言 | 13 项差距全部登记（P0×2 / P1×6 / P2×5）；GAP-S01 ✅ 已处理（v2.61.0：EvolutionLoop stock 分支自动注入 + 键归一化 + ic_pre_neutral 对比，死配置 `stock_neutralization` 已接通）；GAP-S02 ✅ 已处理（v2.62.0：barra 包 10 风格暴露引擎 + 逐日 OLS 回归残差 + 评估链 Step 2.6 两级中性化链，test_barra.py 13 用例）；GAP-S09 种子表达式统一走 DSL 编译链或双路一致性校验；GAP-S11 股票演化 operator-first；实施按 §2 路线图推进；每项含代码依据与测试方案 |
