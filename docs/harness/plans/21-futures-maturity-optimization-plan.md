...
| 版本 | 阶段 | 缺陷项 | 内容 |
|---|---|---|---|
| v2.59.0 | 阶段 B | GAP-F03 优先 | 期货截面因子板块中性化主流程（P0 中风险最低、收益最直接） |
| v2.59.0 | 阶段 B | GAP-F02 | 回测涨跌停拦截 + 停牌过滤 + 被拦截成交统计 |
| v2.60.0 | 阶段 C | GAP-F01 | 实盘执行链路（live_trade 骨架 + 订单状态机 + 止损单 + 人工干预 + 灰度） |
| v2.60.0 | 阶段 C | GAP-F08 | 样本外强制（晋升路径强制 WalkForward + 配置开关 + OOS 报告） |
| v2.60.0 | 阶段 C | GAP-F09 | 保证金建模（保证金率表 + CapitalAllocator 约束 + 强平告警） |
...
| v2.60.0 | 阶段 C | GAP-F11 | 展期成本联动换月日历（后续批） |
