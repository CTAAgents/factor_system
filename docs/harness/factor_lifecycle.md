...
> **调度源（v2.104.0+98）**：内部 `fts/scheduler` 定时任务已全部停用（enabled=False），上表周期任务由 **TRAE Schedule 定时自动化**执行（各任务已建对应自动化，时间与上表一致），内部 `SchedulerEngine.list_enabled()` 返回空、不重复执行。
