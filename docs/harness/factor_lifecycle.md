...
| L2 演化 | 工作日 04:00（≈10 代）/ 周六 04:00（≈50 代） | GP 演化 + UCT 选择 → 晋升 elite |
...
| 巡检/监控 | 每日 03:00 巡检降级 · 22:00 逻辑监控 · 04:00 数据级监控 | 退化检测 / 行为漂移 / 数据质量 |
...
| 机制 | 实现/调度 | 规则 |
|:-----|:----------|:-----|
| 因子巡检降级 | `factor_inspector_job`（每日 03:00） | 扫描 elite 退化因子（质量分/IC 衰减），检测退化 → `status='degraded'` + `is_elite=False` |
| 逻辑监控 | `logic_monitor_job`（每日 22:00） | 行为漂移 / 极端预测 / 换月日异常 |
