...
```
v0.1.0 ───→ v0.2.0 ───→ v0.3.0 ───→ v1.1.0 ───→ v1.2.0 ───→ v1.5.0 ───→ v1.6.0 ───→ v1.7.0 ───→ v1.8.0 ───→ v1.8.1 ───→ v1.9.0 ───→ v1.10.0 ───→ v1.11.0 ───→ v2.0.0 ───→ v2.104.0 ───→ v2.105.0 (当前)
    │          │          │          │          │          │          │          │          │          │          │          │          │          │          │          │
    ├ Phase    ├ CLI 真实  ├ Data-Core ├ MCP 迁移  ├ 种子因子  ├ 期货数据  ├ 期货自治循环├ 策略进化  ├ 信号管道v5├ 演化优化  ├ Phase A  ├ Phase B  ├ Phase C  ├ 多周期IC ├ 35-gap  ├ 子链张量化四层闭环
    │ 1-7 完成 │ 调用      │ 集成      │ (akshare) │ 集成      │ 接入      │ L1/L2/L3 调度├ 动态因子权重├ 多空双向排名├ UCT 父因子├ 消融实验  ├ SHAP 分析  ├ 因果结构审查├ 盲测池  ├ 全链路  ├ 评审质检/生命周期/组合权重/信号管道
    ├ 220 测试 ├ Scheduler ├ FDT 清除  ├ 移除期货  ├ 熔断修复  ├ DuckDB +  ├ 期货基本面  ├ 市场制度  ├ 信号增量  ├ 失败模式  ├ 场景测试  ├ 鲁棒性审查  ├ 持续监控仪表盘├ 机构标准├ G1-G17 ├ 评估单元 (factor, market, chain)
    │ 全绿     │ 引擎      │           │ 种子      │           │ AKShare   │ 数据接入    ├ 自适应     ├ 信号快照   ├ 聚类       ├ 风险标签  ├ 34 新测试  ├ 40 新测试  ├ 9 新测试 ├ 缺口关闭├ 全链路灰度默认关
    └ 71% 覆盖 ├ 89% 覆盖  ├ 原子持久化 ├ 1231 测试 ├ 纯多头    ├ 82 期货   ├ 信号管道    ├ 1601 测试 ├ 1700 测试 ├ 1750 测试 ├ 1650 测试 ├ 99% 覆盖  ├ 99% 覆盖  └ 全绿      └ 全绿      └ 143 新用例全绿
               └ 778 全绿  ├ 96% 覆盖  ├ 99% 覆盖  │ 回测      │ 品种      │ 定时任务    └ 全绿      └ 全绿      └ 全绿      └ 全绿
...
**完成时间**: 2026-08-17（正式版本发布：v2.104.0+115 → v2.105.0，build 段清零）

**架构意义**: 因子评估单元从"全链单值"升级为"(factor_id, market, chain)"张量——**评审质检、生命周期、组合权重、信号管道全链路子链差异化**，评估单元参数化（`config/futures_universe.yaml` 加子链映射即扩展至黑色/有色/农产品/金融等其它产业链），四层矩阵同网格正交：

```
子链 C1    C2    C3    C4
g[子链]          方向层   能/不能做   Regime Gate（plans/48，信号层硬 Gate + 权重层 gate_scale 闭环）
m[factor][子链]  幅度层   做多少      调制矩阵（plans/47，zero/soft 双模式）
q[factor][子链]  质量层   值不值得做  评审质检+生命周期张量化（plans/49）
```

**核心产出**:
- ✅ **plans/47 调制矩阵 m**（v2.104.0+109）：子链画像+三门槛显著性护栏（`subchain_profile.py`）+ L3 子链差异化权重（`subchain_weight.py`，zero/soft）+ 期货/股票评估口径分离（`get_eval_config`）+ 子链暴露监控
- ✅ **plans/48 方向 Gate g**（v2.104.0+111）：子链方向 Gate（`regime_gate.py`，信号管道 Step 3h1）+ 品种暴露缩放（置信度映射×对齐度）+ 收益来源族激活（子链 regime 路由）
- ✅ **plans/49 质量矩阵 q**（v2.104.0+112）：`subchain_factor_quality` 时序表 + Q10/F6 两级判定 + 机审单链特异放行 + 单元粒度退化检测 + scope 动态收缩闭环
- ✅ **plans/50 权重源头 Gate 闭环**（v2.104.0+113）：`gate_scale_map` + `_merge_gate_scale_into_modulation`——avoid 链在权重源头归零/降权（m' = m × gate_scale），质量报告 `subchain_gate_scale` 段四网合一
- ✅ 全链路灰度：`l3.subchain_weight` / `l3.regime_gating` / `l3.subchain_quality` 全部默认关，显式打开逐层生效，向后兼容零行为变更
- ✅ 测试：plans/47-50 累计新增 143 用例（50+36+46+11）全绿 + 受影响回归全绿 + ruff 通过；GAP-134/136/137/138 登记并关闭
...
| 可验证断言 | 当前版本 **v2.105.0**（2026-08-17 架构里程碑：子链张量化四层闭环 plans/47/48/49/50）已登记；晋级总览版本链以 `v2.105.0 (当前)` 结尾；v2.60.0 登记 GAP-I 系列总纲（plans/23）+ v2.61.0 登记 L3/L4 专项 A 阶段（plans/24，GAP-L301/L302/L305 处理中）；GAP-134/136/137/138 已关闭、GAP-140（93 预存失败专项）开放 |
| 检验方式 | 检查本文件下阶段目标表和版本历史确认当前版本和路线图；`python -c "import fts; assert fts.__version__ == '2.105.0'"` |