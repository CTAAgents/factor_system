...
```
v0.1.0 ───→ v0.2.0 ───→ v0.3.0 ───→ v1.1.0 ───→ v1.2.0 ───→ v1.5.0 ───→ v1.6.0 ───→ v1.7.0 ───→ v1.8.0 ───→ v1.8.1 (当前)
    │          │          │          │          │          │          │          │          │
    ├ Phase    ├ CLI 真实  ├ Data-Core ├ MCP 迁移  ├ 种子因子  ├ 期货数据  ├ 期货自治循环├ 策略进化  ├ 信号管道v5
    │ 1-7 完成 │ 调用      │ 集成      │ (akshare) │ 集成      │ 接入      │ L1/L2/L3 调度├ 动态因子权重├ 多空双向排名
    ├ 220 测试 ├ Scheduler ├ FDT 清除  ├ 移除期货  ├ 熔断修复  ├ DuckDB +  ├ 期货基本面  ├ 市场制度自适应├ 信号增量追踪
    │ 全绿     │ 引擎      │           │ 种子      │           │ AKShare   │ 数据接入    └ 多周期信号融合├ 信号快照持久化
