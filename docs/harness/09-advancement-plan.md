...
```
v0.1.0 ───→ v0.2.0 ───→ v0.3.0 ───→ v1.1.0 ───→ v1.2.0 ───→ v1.5.0 ───→ v1.6.0 ───→ v1.7.0 ───→ v1.8.0 ───→ v1.8.1 ───→ v1.9.0 (当前)
    │          │          │          │          │          │          │          │          │          │
    ├ Phase    ├ CLI 真实  ├ Data-Core ├ MCP 迁移  ├ 种子因子  ├ 期货数据  ├ 期货自治循环├ 策略进化  ├ 信号管道v5├ 演化优化
    │ 1-7 完成 │ 调用      │ 集成      │ (akshare) │ 集成      │ 接入      │ L1/L2/L3 调度├ 动态因子权重├ 多空双向排名├ UCT 父因子选择
    ├ 220 测试 ├ Scheduler ├ FDT 清除  ├ 移除期货  ├ 熔断修复  ├ DuckDB +  ├ 期货基本面  ├ 市场制度自适应├ 信号增量追踪├ 失败模式聚类
    │ 全绿     │ 引擎      │           │ 种子      │           │ AKShare   │ 数据接入    └ 多周期信号融合├ 信号快照持久化├ 32 新测试
    └ 71% 覆盖 ├ 89% 覆盖  ├ 原子持久化 ├ 1231 测试 ├ 纯多头    ├ 82 期货   ├ 信号管道定时                      ├ 1601 测试全绿
               └ 778 全绿  ├ 96% 覆盖  ├ 99% 覆盖  │ 回测      │ 品种      │ 任务                              └ README拆分因子
...
| 可验证断言 | 当前版本 v1.9.0 里程碑已登记，v2.0.0 按路线图推进 |
